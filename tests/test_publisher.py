...
from publisher.publisher import instance_publisher
...
class TestInstancePublisher(unittest.TestCase):
    ...
    @patch("uuid.uuid4")
    @patch("utils.time_randomizer.TimeRandomizer")
    @patch("time.sleep")
    def test_instance_publisher(self, mock_sleep, mock_time_randomizer, mock_uuid, mock_height_gen,
        ...
        mock_uuid.return_value = MagicMock(hex="test-instance-123")

        mock_sock = MagicMock()
        mock_socket.return_value = mock_sock

        mock_packet_generator_instance = MagicMock()
        mock_packet_generator_instance.generate_geo_location_packet.side_effect = [
            {
        ...
        self.assertEqual(sent_address, ("255.255.255.255", 5005))
